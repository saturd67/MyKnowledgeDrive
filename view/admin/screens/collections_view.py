...
def build(portal):
    p = palette()
    rows = _filtered(portal)
    page_size = portal.state["page_size"]
    total_pages = max(1, -(-len(rows) // page_size))
    current = min(portal.state["collections_page"], total_pages)
    portal.state["collections_page"] = current
    ...
    return ft.Column(
        [
            w.page_header(
                ...
                actions=[
                    w.ghost_button(
                        ...
                        on_click=lambda _: portal.not_implemented("Refresh collections"),
                    ),
                ],
            ),
            ...
            ft.Row(
                [
                    w.stat_card(ft.Icons.STORAGE_ROUNDED, "Total documents", str(len(data.DOCUMENTS)), None, "primary"),
                    w.stat_card(ft.Icons.FILTER_ALT_ROUNDED, "Matching filter", str(len(rows)), None, "info"),
                    w.stat_card(ft.Icons.LAYERS_ROUNDED, "Pages", str(total_pages), f"{page_size} per page", "warning"),
                ],
                spacing=Space.LG,
            ),
            ...
            w.section(
                ...
                content=ft.Column(
                    [
                        _table(portal, visible, current, page_size),
                        ft.Container(height=Space.LG),
                        _pager(portal, current, total_pages, len(rows)),
                    ],
                    spacing=0,
                ),
            ),
...
def _filtered(portal):
    needle = portal.state["collections_filter"].strip().lower()
    if not needle:
...
def _toolbar(portal):
    ...
    def on_filter(e):
        portal.state["collections_filter"] = e.control.value
        portal.state["collections_page"] = 1
        portal.refresh()

    def on_page_size(e):
        portal.state["page_size"] = int(e.control.value)
        portal.state["collections_page"] = 1
        portal.refresh()

    search = ft.TextField(
        value=portal.state["collections_filter"],
    ...
    size = ft.Container(
        content=ft.Dropdown(
            value=str(portal.state["page_size"]),
...
def _table(portal, visible, current, page_size):
    p = palette()
    ...
    rows = [header, ft.Container(height=1, bgcolor=p.border_soft)]
    start = (current - 1) * page_size

    for offset, (doc_id, doc_label, kind) in enumerate(visible):
        rows.append(_row(portal, start + offset + 1, doc_id, doc_label, kind))

    return ft.Column(rows, spacing=0)


def _row(portal, number, doc_id, doc_label, kind):
    p = palette()
    ...
    return ft.Container(
        content=ft.Row(
            [
                ...
                ft.Container(content=w.pill(kind, _kind_tone(kind)), width=80),
                ft.Row(
                    [
                        w.icon_button(ft.Icons.CONTENT_COPY_ROUNDED, "Copy document id",
                                      lambda _: portal.not_implemented("Copy document id")),
                        w.icon_button(ft.Icons.OPEN_IN_NEW_ROUNDED, "Open in Google Drive",
                                      lambda _: portal.not_implemented("Open in Google Drive")),
                    ],
                    spacing=0,
                    width=88,
                ),
...
def _pager(portal, current, total_pages, total_rows):
    p = palette()

    def go(delta):
        def handler(_):
            portal.state["collections_page"] = max(1, min(total_pages, current + delta))
            portal.refresh()
        return handler

    def jump(index):
        def handler(_):
            portal.state["collections_page"] = index
            portal.refresh()
        return handler
